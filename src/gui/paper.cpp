...
#include <iostream>
// Paper constructor.
// Here we are setting the DPI resolution which we will be using for the paper
// and rendering We then calculate the paper size in pixels using the
// measurements stored in QPageSize enums After we do the calculations, we draw
// a rectangle on screen using the provided measurements and set it as the scene
// rectangle.
...
QRectF Paper::getPaperRect() const { return paperRectangle; }
...
void Paper::setPaperRect(QRectF newRect) { paperRectangle = newRect; }
...
void Paper::drawPaper() { rectItem = this->addRect(paperRectangle, QPen(), QBrush(Qt::white)); }
...
void Paper::dragMoveEvent(QGraphicsSceneDragDropEvent *event) { event->acceptProposedAction(); }

int Paper::getRes() const { return res; }

void Paper::setRes(int newRes) { res = newRes; }

double Paper::getMarginTop() const { return marginTop; }

void Paper::setMarginTop(double newMarginTop) { marginTop = newMarginTop; }

double Paper::getMarginBottom() const { return marginBottom; }

void Paper::setMarginBottom(double newMarginBottom) { marginBottom = newMarginBottom; }

double Paper::getMarginLeft() const { return marginLeft; }

void Paper::setMarginLeft(double newMarginLeft) { marginLeft = newMarginLeft; }

double Paper::getMarginRight() const { return marginRight; }

void Paper::setMarginRight(double newMarginRight) { marginRight = newMarginRight; }

const QPageSize &Paper::getPageSize() const { return pageSize; }

void Paper::setPageSize(const QPageSize &newPageSize) { pageSize = newPageSize; }
...
void Paper::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    if (itemToRemove == nullptr or itemToRemove->type() == QGraphicsRectItem::Type or
        itemToRemove->type() == PageGrid::Type)
    {
        return;
    }
...
PageGrid *Paper::getPageGrid() const { return pageGrid; }

int Paper::fromMMtoPixel(double mm) { return qRound(mm * (res / 25.4)); }
