...
/** position must be valid. if the position exceedes length returns the last
 * page, or first if position is negative */
...
Paper *Document::getCurrentPage() { return this->papers[this->currentPage]; }
...
Paper *Document::getLastPage() { return this->papers.last(); }
...
void Document::nextPage() { updatePageChanged(this->currentPage + 1); }

void Document::prevPage() { updatePageChanged(this->currentPage - 1); }
...
void Document::slotUpdateItemGeometry(QGraphicsItem *item) { emit signalUpdateItemGeometry(item); }
...
/**
 * TODO: trebalo bi da se na osnovu glavnih podesavanja dokumenta, tj papira
 * kreira i svaki drugi isti
 */
Paper *Document::getNewPage()
{
    Paper *newPage = new Paper(QPageSize::A4, 300, nullptr);
    connect(newPage, SIGNAL(focusItemChanged(QGraphicsItem *, QGraphicsItem *, Qt::FocusReason)), this,
            SLOT(slotFocusItemChanged(QGraphicsItem *, QGraphicsItem *, Qt::FocusReason)));
    connect(newPage, &Paper::itemRemoved, this, &Document::slotItemRemoved);
    connect(this, SIGNAL(signalUpdateItemGeometry(QGraphicsItem *)), newPage,
            SLOT(updateItemGeometry(QGraphicsItem *)));
    return newPage;
}
