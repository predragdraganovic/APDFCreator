...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(document, SIGNAL(signalFocusItemChanged(QGraphicsItem *, QGraphicsItem *, Qt::FocusReason)), toolOpts,
            SLOT(componentFocused(QGraphicsItem *, QGraphicsItem *)));
...
void MainWindow::setNewPageScene(int newPageIndex)
{
    ...
    connect(currentPage, SIGNAL(focusItemChanged(QGraphicsItem *, QGraphicsItem *, Qt::FocusReason)), toolOpts,
            SLOT(componentFocused(QGraphicsItem *, QGraphicsItem *)));
...
void MainWindow::showEvent(QShowEvent *e)
{
    if (firstShow)
    {
        ...
        ui->graphicsView->fitInView(0, 0, document->getCurrentPage()->width(), document->getCurrentPage()->height(),
                                    Qt::KeepAspectRatio);
    }
    QWidget::showEvent(e);
}
...
// This is done by grabbing the rendered scene and providing it to the painter
// which we provide to the printer.

void MainWindow::on_actionExport_triggered() { InputOutput::exportToPdf(*document); }

// Creating a QPrinter object used for printing to a real printer. This is done
// by calling the system print dialog and sending the rendered graphics scene as
// the data input

void MainWindow::on_actionPrint_triggered() { InputOutput::printDocument(*document); }
...
void MainWindow::on_actionSave_triggered() { io->saveToApdf(*document); }
...
void MainWindow::on_PaperRedrawn() { zoomSpinBox->setValue(100); }
