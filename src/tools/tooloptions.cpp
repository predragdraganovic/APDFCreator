...
QGroupBox *ToolOptions::getComponentOptionsHolder() const { return componentOptionsHolder; }
...
void ToolOptions::setTextToolOptions(TextTool &current)
{
    ...
    for (int i = 0; i < componentOptionsHolder->layout()->count(); i++)
    {
        ...
        else if (componentOptionsHolder->layout()->itemAt(i)->widget()->objectName() == "fontSizes")
        {
            ...
            fontSize->setCurrentText(
                QString::number(qRound(current.getTextDocument()->defaultFont().pointSize() / (300 / 72.0))));
        }
    }
...
int ToolOptions::getPaperHeight() const { return paperHeight; }

void ToolOptions::setPaperHeight(int newPaperHeight) { paperHeight = newPaperHeight; }

int ToolOptions::getPaperWidth() const { return paperWidth; }

void ToolOptions::setPaperWidth(int newPaperWidth) { paperWidth = newPaperWidth; }
