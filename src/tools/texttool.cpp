...
// It is impossible to move items with TextEditorInteraction due to Qt
// limitations.
...
QString TextTool::getTextData() const { return textDocument->toMarkdown(); }

void TextTool::setTextData(const QString &text) { textDocument->setMarkdown(text); }

QTextDocument *TextTool::getTextDocument() const { return textDocument; }
...
QVBoxLayout *TextTool::getToolOptions() const
{
    ...
    fontSizes->addItems(QStringList() << "8"
                                      << "9"
                                      << "10"
                                      << "11"
                                      << "12"
